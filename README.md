...
- **プレイリスト再生**:
  - 作成したプレイリストを連続再生
  - **リピート再生**: 1曲リピート、全曲リピートに対応
  - **シャッフル再生**: プレイリストをランダムな順序で再生
- **プレイリスト管理**: プレイリストアイテムの追加、編集、削除、並び替え
- **データ保存**: SharedPreferencesにプレイリストを保存
- **CSVエクスポート/インポート**: プレイリストをCSV形式でエクスポート・インポート
- **YouTube URLパーサー**: YouTubeのURLから動画IDを自動抽出
- **レスポンシブUI**: PC、スマートフォンなど、さまざまな画面サイズに最適化
- **ユーザーマニュアル**: アプリ内から操作マニュアルにアクセス可能
- **Firebase Analytics**: アプリの使用状況を追跡（オプション）
...
**オプション**: `script/fetch_comments.py`を使用してYouTube動画のコメントからタイムスタンプを抽出することもできます。
...
1. **再生コントロール**:
   - ▶️ **再生**: プレイリストの連続再生を開始します。
   - ⏹️ **停止**: 連続再生を停止します。
   - 🔁 **リピート**: 再生モードを「リピートなし」→「1曲リピート」→「全曲リピート」の順に切り替えます。
   - 🔀 **シャッフル**: プレイリストのシャッフル再生を有効/無効にします。
2. **個別再生**:
   - プレイリストからアイテムを選択すると、その曲が再生されます。
3. **ヘッダーアイコン**:
   - 🔄 **リロード** (Web版のみ): ページを再読み込みします。
   - ❓ **ユーザーマニュアル**: 操作方法を記載したマニュアルを開きます。
...
│   │   └── widgets/                 # ウィジェット
│   │       └── youtube_list_download_dialog.dart  # YouTubeリストダウンロードダイアログ
│   ├── domain/                      # ドメイン層
│   │   ├── entities/                # エンティティ
...
│   │   │   └── desktop_player.dart  # デスクトッププレイヤー
│   │   └── stubs/                   # プラットフォームスタブ
│   └── core/                        # コアユーティリティ
│       ├── config/                  # 設定
│       │   └── api_config.dart      # API設定
│       ├── services/                # サービス
│       │   └── analytics_service.dart  # アナリティクスサービス
│       └── utils/                   # ユーティリティ
│           ├── csv_export.dart      # CSVエクスポート
│           ├── csv_import.dart      # CSVインポート
│           ├── time_format.dart     # 時間フォーマット
│           └── youtube_url_parser.dart  # YouTube URLパーサー
├── assets/
│   └── logo.png                     # アプリケーションロゴ
├── docs/
│   └── index.html                   # ユーザーマニュアル
├── script/
│   ├── fetch_videos.py              # YouTube動画情報取得スクリプト
│   ├── fetch_comments.py            # YouTubeコメント取得スクリプト
...
- `webview_flutter`: デスクトッププラットフォームでのYouTube動画再生
- `path_provider`: ファイルパスの取得
- `package_info_plus`: アプリ情報の取得
- `firebase_core`: Firebaseコア機能
